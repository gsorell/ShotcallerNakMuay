...
export default function PageLayout({ title, children, onBack }: { title: string; children: React.ReactNode; onBack?: () => void }) {
  return (
    <>
      <style>{`
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: '#1e0b30', backdropFilter: 'blur(10px)', padding: '1rem 2rem', borderBottom: '1px solid rgba(236,72,153,0.3)', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {onBack && (
        <div style={{ display: 'flex', justifyContent: 'center', padding: '0.75rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
          <button onClick={onBack} style={linkButtonStyle}>&larr; Back to Timer</button>
        </div>
      )}
      ...
      <footer style={{ textAlign: 'center', padding: '2rem', borderTop: '1px solid rgba(255,255,255,0.1)', color: '#fdf2f8' }}>
